##################functions for Condatis as 25YEP indicator######################
#####Jenny Hodgson 20 July 2020
...
st4directions<-function(regionb){
#input is assumed to be a raster defining a boundary (1s) around a non-disjoint region (0s), surrounded by NAs

bpt<-rasterToPoints(regionb,fun=function(x){x>0},spatial=F)

#select the most southerly point on the oundary
...
speednflow<-function(apt,sourcept,targetpt,R,disp,cellside,speedonly=TRUE){
		#condatis overall speed and the flow through each habitat cell
